...
def partition_data(dataname, datadir, partition, n_nets, alpha,num_class):
    ...
    idxs1=[[],[],[],[],[],[],[],[],[],[]]
    idxs1=[[]for i in range(num_class)]
    for i in range(n_train):
        idxs1[train_data_targets[i]].append(i)
    ...
    if len(poison_idx):
        if dataname == 'mnist':
            ...
            for idx in poison_idx:
                data_for_poison.targets[idx] = trigger_label
                for c in range(channels):
                    data_for_poison.data[idx, width - 3, height - 1, c] = 255
                    data_for_poison.data[idx, width - 3, height - 2, c] = 255
                    data_for_poison.data[idx, width - 3, height - 3, c] = 255
                    data_for_poison.data[idx, width - 1, height - 3, c] = 255
...
def create_train_data_loader(dataname, train_data, trigger_label, posioned_portion, batch_size, dataidxs, malicious=True,drop_last=True):
    if malicious == True:
        ...
        train_data_loader = DataLoader(dataset=whole_data, batch_size=batch_size, shuffle=True,drop_last=drop_last)
        if len(clean_idx):
            clean_part_load = DataLoader(dataset=clean_part_data, batch_size=batch_size, shuffle=True,drop_last=drop_last)
        else:
            clean_part_load = []
        if len(poison_idx):
            poison_part_loader = DataLoader(dataset=poison_part_data, batch_size=batch_size, shuffle=True,drop_last=drop_last)
        else:
            poison_part_loader = []
        ...
        train_data_loader = DataLoader(dataset=train_data_client, batch_size=batch_size, shuffle=True,drop_last=drop_last)
        return train_data_loader

def create_test_data_loader(dataname, test_data, trigger_label, batch_size,drop_last=True):
    ...
    test_data_tri_loader = DataLoader(dataset=test_data, batch_size=batch_size, shuffle=True,drop_last=drop_last)
    test_data_ori_loader = DataLoader(dataset=test_data_ori, batch_size=batch_size, shuffle=True,drop_last=drop_last)
