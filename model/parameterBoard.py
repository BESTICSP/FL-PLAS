...
if __name__ == "__main__":
    ...
    parser.add_argument('--cut', type=int, default=12,help='defense method used: none|krum|multi-krum|xmam|ndc|rsa|rfa|')
    ...
    if args.model == "lenet":
        if args.load_premodel==True:
            ...
            with open("savedModel/mnist_hetero-dir_.pt", "rb") as ckpt_file:
        ...
        if args.load_premodel==True:
            ...
            with open("savedModel/{}_{}_.pt".format(args.dataname,args.partition_strategy), "rb") as ckpt_file:
                ckpt_state_dict = torch.load(ckpt_file, map_location=device)
                # net_avg.load_state_dict(ckpt_state_dict)
                # net_avg.classifier = nn.Sequential(
        ...
        # net_avg = ResNet18(args.num_class).to(device)
        ...
        # net_avg = ResNet18(1000).to(device)
        # net_avg.fc = nn.Linear(512, args.num_class).to(device)
        if args.load_premodel==True:
            with open("savedModel/cifar100_hetero-dir_.pt", "rb") as ckpt_file:
                ckpt_state_dict = torch.load(ckpt_file, map_location=device)
            net_avg.load_state_dict(ckpt_state_dict)
            logger.info("Loading pre-model successfully ...")

    elif args.model =='resnet50':
        net_avg = models.resnet50(pretrained=True).to(device)
        net_avg.avgpool = nn.AdaptiveAvgPool2d(1).to(device)
        num_ftrs = net_avg.fc.in_features
        net_avg.fc = nn.Linear(num_ftrs, args.num_class).to(device)
    elif args.model =='dla':
        if args.load_premodel==True:
            net_avg = DLA().to(device)
            with open("savedModel/cifar10_dla_.pt", "rb") as ckpt_file:
                ckpt_state_dict = torch.load(ckpt_file, map_location=device)
            net_avg.load_state_dict(ckpt_state_dict)
            logger.info("Loading pre-model successfully ...")
        else:
